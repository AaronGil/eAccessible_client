...
/**
 * Servlet implementation class SvlVerificar
 */
@WebServlet("/SvlVerificar")
public class SvlVerificar extends HttpServlet {
	...
	private void doFer(HttpServletRequest request, HttpServletResponse response) {
		...
		try{
			...
			port.validacioLocal(Integer.parseInt(codiLocal));
		}
		catch (Exception e) { 
			e.printStackTrace();
		}
		...
		try{
			...
			RequestDispatcher rd = context.getRequestDispatcher("/LocalVerificat");
